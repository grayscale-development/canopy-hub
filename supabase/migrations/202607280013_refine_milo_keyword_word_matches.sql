...
as $$
  ...
  significant_terms as (
    select distinct
      term,
      '(^|[^a-z0-9])' || term || '([^a-z0-9]|$)' as term_pattern
        ...
        (
          select max(
            case
              ...
              when lower(rc.source_title) ~ term_pattern then 0.75
              when lower(coalesce(rc.source_url, '')) ~ term_pattern then 0.65
              when lower(rc.search_text) ~ term_pattern then 0.35
              else 0
            end
          )
  ...
  where to_tsvector('english', rc.search_text) @@ rc.exact_query
    ...
    or exists (
      select 1
      from significant_terms
      where lower(rc.search_text) ~ term_pattern
    )
